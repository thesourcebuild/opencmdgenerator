...
import type { KillPlatform, KillSpec } from "@cmdgen/kill";
import { buildArgv, lint, renderTokens } from "@cmdgen/kill";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./kill-target-selector";

const CONTINUATION: Record<KillPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-powershell": " `",
  "windows-cygwin": " \\",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `KillTargetSelector`, not here. */
...
export interface KillPreviewProps {
  spec: KillSpec;
  onPlatformChange: (platform: KillPlatform) => void;
}

/** kill's data for the shared `GeneratedCommandPanel` template — one variant, plus a platform picker (POSIX kill vs PowerShell's Stop-Process — no signal concept at all on the PowerShell side). */
export function KillPreview({ spec, onPlatformChange }: KillPreviewProps) {
    ...
    <GeneratedCommandPanel
      description={
        spec.platform === "windows-powershell"
          ? "This app never stops any process."
          : "This app never sends any signal."
      }
