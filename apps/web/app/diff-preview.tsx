...
import { buildArgv, lint, renderTokens } from "@cmdgen/diff";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./diff-target-selector";

const CONTINUATION: Record<DiffPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-cmd": " ^",
  "windows-cygwin": " \\",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `DiffTargetSelector`, not here. */
...
export interface DiffPreviewProps {
  spec: DiffSpec;
  onPlatformChange: (next: DiffPlatform) => void;
}

/** diff's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/ls`'s preview, including the always-visible shell picker. */
export function DiffPreview({ spec, onPlatformChange }: DiffPreviewProps) {
