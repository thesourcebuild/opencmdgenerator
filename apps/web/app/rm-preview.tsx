...
import { buildArgv, lint, renderTokens } from "@cmdgen/rm";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./rm-target-selector";

const CONTINUATION: Record<RmPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-powershell": " `",
  "windows-cygwin": " \\",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `RmTargetSelector`, not here. */
...
export interface RmPreviewProps {
  spec: RmSpec;
  onPlatformChange: (next: RmPlatform) => void;
}

/** rm's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/mkdir`'s preview, including the always-visible shell picker. */
export function RmPreview({ spec, onPlatformChange }: RmPreviewProps) {
