...
import { buildArgv, lint, renderTokens } from "@cmdgen/cp";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./cp-target-selector";

const CONTINUATION: Record<CpPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-cmd": " ^",
  "windows-powershell": " `",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `CpTargetSelector`, not here. */
...
export interface CpPreviewProps {
  spec: CpSpec;
  onPlatformChange: (next: CpPlatform) => void;
}

/** cp's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/mv`'s preview. */
export function CpPreview({ spec, onPlatformChange }: CpPreviewProps) {
