...
import { buildArgv, lint, renderTokens } from "@cmdgen/grep";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./grep-target-selector";

const CONTINUATION: Record<GrepPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-cmd": " ^",
  "windows-powershell": " `",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `GrepTargetSelector`, not here. */
...
export interface GrepPreviewProps {
  spec: GrepSpec;
  onPlatformChange: (next: GrepPlatform) => void;
}

/** grep's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/mv`'s preview. */
export function GrepPreview({ spec, onPlatformChange }: GrepPreviewProps) {
