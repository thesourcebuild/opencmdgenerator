...
import { buildArgv, lint, renderTokens } from "@cmdgen/mkdir";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./mkdir-target-selector";

const CONTINUATION: Record<MkdirPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-cmd": " ^",
  "windows-powershell": " `",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `MkdirTargetSelector`, not here. */
...
export interface MkdirPreviewProps {
  spec: MkdirSpec;
  onPlatformChange: (next: MkdirPlatform) => void;
}

/** mkdir's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/cd`'s preview, including the always-visible 3-way shell picker. */
export function MkdirPreview({ spec, onPlatformChange }: MkdirPreviewProps) {
