...
import { buildArgv, lint, renderTokens } from "@cmdgen/mv";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./mv-target-selector";

const CONTINUATION: Record<MvPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-cmd": " ^",
  "windows-powershell": " `",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `MvTargetSelector`, not here. */
...
export interface MvPreviewProps {
  spec: MvSpec;
  onPlatformChange: (next: MvPlatform) => void;
}

/** mv's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/mkdir`/`@cmdgen/ln`'s preview. */
export function MvPreview({ spec, onPlatformChange }: MvPreviewProps) {
