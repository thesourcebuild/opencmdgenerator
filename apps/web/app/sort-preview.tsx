...
import { buildArgv, lint, renderTokens } from "@cmdgen/sort";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./sort-target-selector";

const CONTINUATION: Record<SortPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-cmd": " ^",
  "windows-cygwin": " \\",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `SortTargetSelector`, not here. */
type WindowsShell = "windows-cmd" | "windows-cygwin" | "windows-msys" | "windows-wsl";
...
export interface SortPreviewProps {
  spec: SortSpec;
  onPlatformChange: (next: SortPlatform) => void;
}

/** sort's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/ls`'s preview, including the always-visible shell picker. */
export function SortPreview({ spec, onPlatformChange }: SortPreviewProps) {
