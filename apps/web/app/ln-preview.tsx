...
import type { LnPlatform, LnSpec } from "@cmdgen/ln";
import { buildArgv, lint, renderTokens } from "@cmdgen/ln";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./ln-target-selector";

const CONTINUATION: Record<LnPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-cmd": " ^",
  "windows-powershell": " `",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `LnTargetSelector`, not here. */
...
export interface LnPreviewProps {
  spec: LnSpec;
  onPlatformChange: (next: LnPlatform) => void;
}

/** ln's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/cd`/`@cmdgen/mkdir`'s preview. */
export function LnPreview({ spec, onPlatformChange }: LnPreviewProps) {
