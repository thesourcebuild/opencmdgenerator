...
import type { HeadPlatform, HeadSpec } from "@cmdgen/head";
import { buildArgv, lint, renderTokens } from "@cmdgen/head";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./head-target-selector";

const CONTINUATION: Record<HeadPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-powershell": " `",
  "windows-cygwin": " \\",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `HeadTargetSelector`, not here. */
...
export interface HeadPreviewProps {
  spec: HeadSpec;
  onPlatformChange: (next: HeadPlatform) => void;
}

/** head's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/ls`'s preview, including the always-visible shell picker. */
export function HeadPreview({ spec, onPlatformChange }: HeadPreviewProps) {
