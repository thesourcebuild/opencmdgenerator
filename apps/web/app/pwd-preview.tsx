...
import { buildArgv, lint, renderTokens } from "@cmdgen/pwd";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./pwd-target-selector";

const CONTINUATION: Record<PwdPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-powershell": " `",
  "windows-cygwin": " \\",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `PwdTargetSelector`, not here. */
...
export interface PwdPreviewProps {
  spec: PwdSpec;
  onPlatformChange: (next: PwdPlatform) => void;
}

/** pwd's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/ls`'s preview, including the always-visible shell picker. */
export function PwdPreview({ spec, onPlatformChange }: PwdPreviewProps) {
