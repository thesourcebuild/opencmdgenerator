...
import { buildArgv, lint, renderTokens } from "@cmdgen/clear";
import { GeneratedCommandPanel } from "./generated-command-panel";
import { isWindows } from "./clear-target-selector";

const CONTINUATION: Record<ClearPlatform, string> = {
  linux: " \\",
  mac: " \\",
  "windows-cmd": " ^",
  "windows-powershell": " `",
...
/** Only the Windows sub-choices — "posix" is picked via the Linux/Mac buttons in `ClearTargetSelector`, not here. */
...
export interface ClearPreviewProps {
  spec: ClearSpec;
  onPlatformChange: (next: ClearPlatform) => void;
}

/** clear's data for the shared `GeneratedCommandPanel` template — same shape as `@cmdgen/export`'s preview. */
export function ClearPreview({ spec, onPlatformChange }: ClearPreviewProps) {
