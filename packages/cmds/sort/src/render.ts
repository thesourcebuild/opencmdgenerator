...
import type { SortPlatform } from "./spec";
 ...
 * same shape as `@cmdgen/ls/render`.
 */
export function renderTokens(argv: Argv, platform: SortPlatform): RenderedToken[] {
  return renderTokensGeneric(argv, { shell: toShellDialect(platform) });
}

export function renderOneLine(argv: Argv, platform: SortPlatform): string {
  return renderOneLineGeneric(argv, { shell: toShellDialect(platform) });
}

export function renderMultiLine(argv: Argv, platform: SortPlatform): string {
