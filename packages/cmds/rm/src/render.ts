...
import type { RmPlatform } from "./spec";
 ...
 * down via `toShellDialect` before reusing the generic render pipeline — see
 * the identical note in @cmdgen/ls/render.ts.
 */
export function renderTokens(argv: Argv, platform: RmPlatform): RenderedToken[] {
  return renderTokensGeneric(argv, { shell: toShellDialect(platform) });
}

export function renderOneLine(argv: Argv, platform: RmPlatform): string {
  return renderOneLineGeneric(argv, { shell: toShellDialect(platform) });
}

export function renderMultiLine(argv: Argv, platform: RmPlatform): string {
