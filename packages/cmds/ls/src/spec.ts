 ...
 * (both are plain POSIX `ls`), and Windows further branches into WHICH
 * shell will run the command. Unlike mkdir's 2-way Windows split
 * (windows-cmd/windows-powershell), ls's Windows split is 4-way
 * (windows-powershell/windows-cygwin/windows-msys/windows-wsl) and
 * deliberately excludes a `windows-cmd` value entirely — `ls` has no cmd.exe
 * equivalent binary at all (`dir` is a different tool this app never
 * generates).
 *
 * `windows-cygwin`, `windows-msys`, and `windows-wsl` invoke the exact same
 * real `ls` binary with the exact same flags as `linux`/`mac` — unlike
 * `windows-powershell`'s `Get-ChildItem`, which is a genuinely different
 * program with an almost entirely disjoint flag set (`ls -la` fails outright
 ...
 */
export const LsPlatform = z.enum([
  "linux",
  "mac",
  "windows-powershell",
  "windows-cygwin",
...
export type LsPlatform = z.infer<typeof LsPlatform>;

export const LsSpec = z.object({
  specVersion: z.literal(SPEC_VERSION).default(SPEC_VERSION),
  id: z.string(),
  name: z.string().default(""),

  /** Empty means the current directory — ls's own default. */
  paths: z.array(z.string()).default([]),
  platform: LsPlatform.default("linux"),

  flags: FlagValues.default({}),
});
export type LsSpec = z.infer<typeof LsSpec>;
