 ...
 */
export function renderTokens(argv: Argv, platform: PwdPlatform): RenderedToken[] {
  return renderTokensGeneric(argv, { shell: toShellDialect(platform) });
}

export function renderOneLine(argv: Argv, platform: PwdPlatform): string {
  return renderOneLineGeneric(argv, { shell: toShellDialect(platform) });
}

export function renderMultiLine(argv: Argv, platform: PwdPlatform): string {
