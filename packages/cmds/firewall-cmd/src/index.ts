...
// convenience. Like @cmdgen/ufw, firewall-cmd needs no command-owned
// render.ts: its zone/action/port/service tokens are assembled into plain
// `--flag=value` strings before ever becoming an `Arg` — see
// argv/index.ts — so nothing here needs extra `attached`/`quoteAttached`
// treatment beyond what's already marked.)
export {
  quotePosix,
  quotePowerShell,
  quoteCmd,
...
// identical note in @cmdgen/ufw's index.ts. Reach it via "@cmdgen/firewall-cmd/definition".
