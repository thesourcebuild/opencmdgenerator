...
/**
 * systemctl has zero catalogue flags — its entire syntax here is two bare
 * words, `systemctl ACTION UNIT`, with no `-flag` involved anywhere (see
 * `catalogue/flags.ts`). Kept empty, rather than omitted, so the catalogue
 * machinery's shape stays uniform across every command package in this repo —
 * same reasoning as `@cmdgen/service`'s `catalogue/groups.ts`.
 */
